...
def run_classification(
    transcript_fp: str, dict_fp: str, threshold: float = 0.5
) -> pd.DataFrame:
    """
    ...
    logger.log_info(
        f"Using '{keyword_col}' as keyword column and '{code_col}' as code column"
    )
    ...
    if text_col not in transcript_df.columns:
        ...
        for col in potential_text_columns:
            ...
            raise ValueError(
                f"Cannot find text column in transcript. Required column 'text' or alternatives not found."
            )

    # Process each row with progress bar
    logger.log_info(
        f"Processing {transcript_row_count} rows with classification pipeline"
    )
    for i, row in tqdm(
        transcript_df.iterrows(), total=transcript_row_count, desc="Classifying"
    ):
        try:
            ...
            kw_hits = keyword_match(
                row_for_matching,
                dict_df[[keyword_col, code_col]].rename(
    ...
    logger.log_info(
        f"Processed {transcript_row_count} rows in {duration:.2f} seconds ({transcript_row_count / duration if duration > 0 else 0:.2f} rows/second)"
    )

    # Calculate classification stats
    classified_rows = transcript_df["primary_code"].notna().sum()
    classified_pct = (
        (classified_rows / transcript_row_count) * 100
        if transcript_row_count > 0
        else 0
    )
    logger.log_info(
        f"Successfully classified {classified_rows} rows ({classified_pct:.1f}%)"
    )
