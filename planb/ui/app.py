            ...
            if "primary_confidence" in row.index:
                ...
                if conf is not None:
                    if conf >= 0.8:
                        ...
                        color = (
                            "background-color: #ffeb9c"  # Yellow for medium confidence
                        )
                    else:
...
def main():
            ...
            if transcript_df is not None:
                ...
                if default_text_col not in text_col_options:
                    for col in [
                        ...
                        default_text_col = (
                            text_col_options[0] if text_col_options else None
                        )
                ...
                selected_text_col = st.selectbox(
                    ...
                    index=(
                        text_col_options.index(default_text_col)
                        if default_text_col in text_col_options
                        else 0
                    ),
                    key="text_col",
                )
                ...
                if selected_text_col != "text" and selected_text_col is not None:
                    ...
                    transcript_df = transcript_df.rename(
                        columns={selected_text_col: "text"}
                    )
    ...
    run_button = st.button(
        "Run Classification",
        type="primary",
        disabled=(transcript_df is None or dictionary_df is None),
    )
        ...
        try:
            with st.spinner("Running classification..."):
                ...
                temp_transcript_path = Path(
                    os.path.join(os.path.dirname(__file__), "temp_transcript.csv")
                )
                temp_dict_path = Path(
                    os.path.join(os.path.dirname(__file__), "temp_dict.csv")
                )
                ...
                results_df = run_classification(
                    str(temp_transcript_path), str(temp_dict_path), threshold
                )
